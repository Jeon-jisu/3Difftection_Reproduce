...
class EpipolarWarpOperator(nn.Module):
    ...
    def forward(self, x, source_intrinsics, target_intrinsics, source_pose, target_pose):
        dtype = x.dtype
        if source_intrinsics != None:
            source_intrinsics = source_intrinsics.to(dtype)
        if target_intrinsics != None:
    ...
    def forward(self, x, hint, timesteps, context, source_pose=None, target_pose=None, source_intrinsic=None, target_intrinsic=None,**kwargs):
        ...
        for i,(module, zero_conv) in enumerate(zip(self.input_blocks, self.zero_convs)):
            if guided_hint is not None:
                ...
                h = module(h, emb, context)
            if i in [2,5]:
                # epipolar warp는 semantic controlnet 학습시에는 사용 x
...
class ControlLDM(LatentDiffusion):
    ...
    def __init__(self, use_geo, use_sem, control_stage_config, semantic_control_stage_config, control_key, only_mid_control,*args, **kwargs):
        ...
        # Freeze the Geometric ControlNet and Stable Diffusion
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        if bs is not None:
            control = control[:bs]
        # K 값을 가져와서 source_intrinsic과 target_intrinsic으로 사용
        Ks = batch.get('Ks', None)
        if Ks is not None:
            source_intrinsic = target_intrinsic = Ks.to(self.device)
        else:
            source_intrinsic = target_intrinsic = None
